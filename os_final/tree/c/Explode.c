   ...
   window. */
{
  ...
  /* flush out slide */
  flush(w);
  if (tam_comprimido + (k >> 3))   /* should have read csize bytes, but sometimes */
  {                       /* read one too many:  k>>3 compensates */
    used_csize = lee_entero(eb_central + 16) - tam_comprimido - (k >> 3);
    return 5;
  }
  return 0;
}
