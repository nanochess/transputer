...
{ Revision: Feb/02/2025. Translated game text to English }
...
BEGIN
  ...
  raiz^.pregunta := 'Is it aquatic';      { Primera pregunta           }
  new(nodo);                              { Crea el descendiente izq.  }
  raiz^.izquierdo := nodo;
  nodo^.pregunta := 'whale';              { Respuesta para un "SI"     }
  nodo^.izquierdo := NIL;
  nodo^.derecho := NIL;
  new(nodo);                              { Crea el descendiente der.  }
  raiz^.derecho := nodo;
  nodo^.pregunta := 'lion';               { Respuesta para un "NO"     }
  nodo^.izquierdo := NIL;
  nodo^.derecho := NIL;
...
BEGIN
  writeln;
  writeln('A N I M A L S. Ver. 23-04-96');
...
{                   --------------------------------                   }
{                                                                      }
{ Acepta respuestas "YES", "NO", "Y" y "N".                             }
...
BEGIN
  ...
  IF (entrada = 'YES') OR (entrada = 'Y') THEN
    checa_respuesta := SI
...
BEGIN
  ...
  write('I surrender. What''s your animal');
  ...
  writeln('Introduce a question that distinguishes ');
  ...
  writeln;
  writeln('Now I know ',numero_animales:0,' different animals!');
  writeln
END;
...
BEGIN
  writeln;
  IF ya_jugo THEN
    writeln('Thanks for playing Animals')
  ELSE
    writeln('Try it someday');
  writeln;
  limpia_memoria(raiz)
END;
